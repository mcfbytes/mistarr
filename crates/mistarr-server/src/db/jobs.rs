...
const COLUMNS: &str = "id, kind, payload, state, progress, created_at, updated_at";
...
/// let id = jobs::insert(&conn, "scan", &serde_json::json!({}), 1).unwrap();
/// assert_eq!(jobs::get(&conn, id).unwrap().unwrap().kind, "scan");
/// ```
pub fn insert(conn: &Connection, kind: &str, payload: &Value, now: i64) -> Result<JobId> {
    conn.execute(
        "INSERT INTO jobs (kind, payload, state, created_at, updated_at) VALUES (?1, ?2, 'queued', ?3, ?3)",
...
/// mistarr_server::db::migrate::apply(&mut conn).unwrap();
/// jobs::insert(&conn, "scan", &serde_json::json!({}), 1).unwrap();
/// assert_eq!(jobs::fail_interrupted(&mut conn, 2).unwrap().len(), 1);
/// ```
pub fn fail_interrupted(conn: &mut Connection, now: i64) -> Result<Vec<JobRow>> {
...
mod tests {
    ...
    fn lifecycle_round_trip() {
        let c = conn();
        let id = insert(&c, "detect_client", &json!({"a": 1}), 10).expect("insert");
    ...
    fn active_listing_pages_and_skips_finished() {
        let c = conn();
        let ids: Vec<_> = (0..3)
            .map(|i| insert(&c, "scan", &json!({ "i": i }), 1).expect("insert"))
            .collect();
    ...
    fn interrupted_jobs_fail_and_old_ones_prune() {
        let mut c = conn();
        let a = insert(&c, "scan", &json!({}), 1).expect("insert");
        let b = insert(&c, "scan", &json!({}), 1).expect("insert");
        set_state(&c, b, JobState::Done, 1).expect("done");
        let failed = fail_interrupted(&mut c, 5).expect("fail");
        assert_eq!(failed.iter().map(|r| r.id).collect::<Vec<_>>(), [a]);
        assert_eq!(
