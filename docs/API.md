...
files it gave a rom. A running `chd_tracks` job reports `{ platform_id, done,
total, file, bytes_done, bytes_total }` at most every 2 s, `done` and `total`
counting images and the bytes those of the image being decoded; a finished
one `{ done, total, verified, unmatched, not_identified }`.
