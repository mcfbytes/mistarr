...
| WP-30 | DAT export format, DAT screen, large MRAs | Opus | 10, 15, 19, 28 | A No-Intro DB export, plain or zipped, loads as the same game and rom model as a Logiqx DAT, with parents from archive numbers, headerless roms for header-rule platforms named `<game>.<ext>`, and its name from the file name; a proptest holds both forms equal; unknown XML is rejected naming both forms; a DATs screen uploads DATs, lists incoming files with rejection reasons, retries or deletes a rejected file and lists loaded versions; MRAs up to 16 MiB are read with inline part data streamed, never held; `tests/memory.rs` holds a 16 MB export and 16 MRAs of about 3 MB in one batch under budget. |
