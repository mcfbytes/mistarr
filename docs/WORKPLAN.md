...
| WP-41 | Identify CHD images by their tracks | Opus | 11, 26, 35 | A `.chd` on a disc platform is never hashed whole unless a loaded DAT lists a `.chd` rom of its size: the scan reads only its 124-byte header, proven by a reader that fails past byte 124 and by a header-only file that stays `unidentified` with reason `off`, and shows it on the Platforms card as not identified with a reason (`GET /platforms/{id}/unidentified`), not as unmatched; with `[scan] chd_tracks` on (System, labelled slow, default off, runtime-editable, a stored runtime without it never overriding the file), one heavy `chd_tracks` job decodes each image once with an in-tree pure-Rust v5 decoder over flate2, lzma-rs, claxon and ruzstd, rebuilds each track's Redump-style `.bin` (2352 of 2448 bytes, pad frames skipped, audio little-endian, stored pregaps included) into the existing hasher with nothing written to disk, verifies per-hunk CRC16 and the raw and combined header SHA1, caches per-track hashes on header SHA1 plus size, and writes `g.chd#NN` members plus `g.chd#cue`, verified only when every track of one DAT entry matches a distinct rom; cooked tracks, unstored pregaps, parents, GD-ROM, unsupported codecs, old track lists and damaged images are not identified with their reason and not decoded again; the job pauses between 32-hunk slices while a core runs and yields the heavy lane only to other job kinds; the launch hands over the `.chd`; synthetic discs encoded by chdman (default codecs, cdzs, cdfl, cdzl, small and 3-byte-length hunks; split and single-file cues; Mode 1, Mode 2 and audio tracks with INDEX 00 pregaps) decode to their source bins' hashes, required in CI; the in-repo synthetic writer covers every codec, real MAME-canonical Huffman maps, and agrees with chd-rs; KATs from docs/CHD.md pin the map, canonical codes and CHT2 layout; a proptest over track layouts, hunk sizes and codecs holds; mutated and truncated images fail without panicking; `decode_budget` stays under 24 MiB at the header limits and `tests/memory.rs` stays under budget; PLATFORMS.md, VERIFICATION.md, UI.md, DATA-MODEL.md, API.md, ARCHITECTURE.md, TESTING.md and the new CHD.md describe it. |
