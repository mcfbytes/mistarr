...
| WP-29 | Arcade scan and counts | Sonnet | 11, 19, 24, 25 | A library scan, its fan-out and the DAT-triggered auto-scan skip the arcade platform, identified by `Platform::is_arcade`; a manual `POST /system/scan` of `arcade` queues only the arcade catalogue job; the catalogue's own presence pass reads every `games/mame`/`games/hbmame` zip's central directory, verifies DAT-sourced titles by CRC32 and size, gives a zip a live MRA names a row to promote, and prunes `files` rows whose zip or member is gone; a migration prunes the noise the old generic scan left behind; `Counts` reports `unmatched_files` for a scan's unmatched files on disk (never arcade) and separate group-based `failing_check`/`partial` arcade-catalogue states; the Platforms screen shows `have · wanted · titles` plus any nonzero extra clause. |
| WP-32 | Fast browse and search | Opus | 10, 15, 23 | `title_groups` is a table the same transaction refreshes through `db::commit`, with a doctor check, `--rebuild-groups`, a migration that builds it and a proptest against the old view SQL; titles' flags, regions and languages are rows of their own tables and nothing filters on JSON; jobs dedupe on `jobs.subject`; search takes the fastest worst case of `LIKE` on the covering index and two trigram FTS5 shapes (one filtered by a sentinel-wrapped platform column), measured on a synthetic catalogue of every major platform's DAT; hidden `bench-seed` and `bench-search` measure the same on the board; every hot read seeks through an index, asserted by a plan test; the Browse screen debounces search, cancels stale requests and reloads, never skips a page after a failure, and shows loading and error states. |
