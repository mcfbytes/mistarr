...
| WP-29 | Arcade scan and counts | Sonnet | 11, 19, 24, 25 | A library scan, its fan-out and the DAT-triggered auto-scan skip the arcade platform, identified by `Platform::is_arcade`; a manual `POST /system/scan` of `arcade` queues only the arcade catalogue job; a migration prunes `files` rows the generic scan wrote for arcade zip members while keeping every row the MRA import path records; `Counts` reports `unmatched_files` for a scan's unmatched files on disk and separate `failing_check`/`partial` arcade-catalogue states; the Platforms screen shows `have · wanted · titles` plus any nonzero extra clause. |
