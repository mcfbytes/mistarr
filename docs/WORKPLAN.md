...
| WP-33 | Safe rollback | Sonnet | 22 | `install.sh` saves `mistarr.db` and any `-wal`/`-shm` as the `mistarr.db.prev` set after stopping mistarr and before overwriting anything, skips it on a fresh install, checks free space and aborts leaving no partial copy when the copy fails, and restores the set with the binary when the new version fails to start; a binary refuses, without writing, a database whose schema version is newer than its migrations, naming both versions, and `doctor` reports it; the shell harness covers the backup, the fresh install, the failed start and the failed backup. |
