...
| WP-34 | Platform art | Opus | 15 | Every platform id draws original abstract SVG art from its motif family, falling back by kind; the same id draws the same art; no image files, network, hardware, logos or brand colours; dark and light variants; the Platforms cards and the Browse header show it `aria-hidden` behind a scrim that keeps AA contrast; under 120 SVG elements per tile; e2e checks each card's art and its determinism. |
