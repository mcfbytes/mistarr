...
| WP-32 | Fast browse and search | Opus | 10, 15, 23 | `title_groups` is a table the same transaction refreshes through `db::commit`, with a doctor check, `--rebuild-groups`, a migration that builds it and a proptest against the old view SQL; titles' flags, regions and languages are rows of their own tables and nothing filters on JSON; jobs dedupe on `jobs.subject`; searches of three or more characters use a trigram FTS5 index; every hot read seeks through an index, asserted by a plan test; a 15 000-title platform browses and searches in well under 100 ms on the host; the Browse screen debounces search, cancels stale requests and shows loading and error states. |
