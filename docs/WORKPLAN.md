...
| WP-43 | DAT import in RAM | Opus | 42 | A DAT import, bind or re-import holds the writer, checkpoints the WAL, checks `MemAvailable` against the copy's need above `[memory] import_floor_mib` and the room in `[memory] import_dir` and on the card, copies the database into RAM with SQLite's backup, runs every member and each loaded platform's recompute there, writes the copy back as `mistarr.db.new` in 1 MiB writes, syncs it, waits while another process holds the database, and renames it over the old file after emptying and removing its WAL, reopening both connections; memory or room short, or running out, imports on the card with the reason in the progress; shutdown, a pause and failures leave the card file byte-identical; startup removes a stale `.new` and working directories and runs pending migrations the same way; `install.sh` guards and removes `.new`; tests count the write-back's syscalls, compare every row with an import on the card, and hold a 450-game load to one card write per MiB plus 16; ARCHITECTURE.md records the counts. |
